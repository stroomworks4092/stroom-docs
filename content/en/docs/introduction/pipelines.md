...
*   **Elements:** The individual "machines" in the factory.
*   **Links:** The "conveyor belts" that connect one element to the next, defining the direction of data flow.
...
1.  **Reader:** Reads the raw bytes from a stream (e.g., `StreamReader`).
1.  **Parser:** Converts the raw text into XML events.
    The most common parser is the `DSParser` (Data Splitter), which uses a "Text Converter" to turn CSV or log files into XML.
1.  **Filter:** Acts on the XML events.
    The most powerful filter is the `XSLTFilter`, which uses XSLT to transform the structure of the data (e.g., renaming fields, adding timestamps, or performing lookups).
1.  **Writer:** Formats the XML events back into a specific format (e.g., `XMLWriter` or `JSONWriter`).
1.  **Appender:** Saves the final output to a destination.
...
You can create a "Base" pipeline that contains common elements (like the Reader and Parser) and then create multiple "Child" pipelines that inherit those elements but add their own specific XSLT transformations.

*   **Benefit:** If you need to change the parsing logic for a specific log format, you change it in the Parent pipeline once, and all Child pipelines are updated automatically.
...
You can see the input and output of every single element in the pipeline simultaneously, making it easy to identify exactly where a transformation might be failing.


## Summary

*   **Pipelines are modular:** You build them by linking elements together.
*   **They are event-driven:** They process data as a stream, not in bulk.
*   **They are reusable:** Use inheritance to share logic across many feeds.
