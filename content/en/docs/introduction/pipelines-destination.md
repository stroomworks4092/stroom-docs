...
This allows for multi-stage processing (e.g., Normalization -> Enrichment -> Indexing).

*   **StreamAppender:** Writes the output as a new stream.
    You must specify:
    *   **Feed:** Which feed the new data belongs to.
    *   **Stream Type:** Typically `Events` (for processed data) or `Context`.
*   **RollingStreamAppender:** Used for high-volume output.
    It pools data and only creates a new stream in the database when a certain size or time threshold is met, reducing database overhead.
...
*   **IndexingFilter (Lucene):** Sends data to a standard Stroom Lucene index.
    The XSLT must produce XML that matches the index's field structure.
*   **ElasticIndexingFilter:** Sends data to an external Elasticsearch cluster.
...
It is used for storing "state" that can be looked up by other pipelines (e.g., "What was the last known IP for this user?").

*   **PlanBFilter:** Instead of writing to a stream or index, this filter writes data into an **LMDB (Lightning Memory-Mapped Database)** shard.
*   **How it works:**
...
*   **Usage:** Other pipelines can then perform ultra-fast lookups against these Plan B maps using the `stroom:lookup()` XSLT function or the `getState()` Dashboard function.
...
*   **HTTPAppender:** Posts the processed data to a remote URL via HTTP or HTTPS.
    This is useful for forwarding alerts to a SOC or a third-party ticketing system.
*   **FileAppender:** Writes the output directly to the local file system of the Stroom node.
*   **HDFSFileAppender:** Writes data to a Hadoop Distributed File System.
